...
pub struct SystemState {
    ...
    pub allowed_internal: Vec<String>, // Whitelist for internal WebView
    pub allowed_browser: Vec<String>,  // Whitelist for system browser
    pub is_dev: bool,
...
pub fn execute_backend(system: &SystemState, trigger: &str, args: &str) -> String {
    if let Some(mut meta) = system.commands.get(trigger).cloned() {
                    ...
                    if let Some(module) = system.modules_map.get(ext) {
                        if let Some(build_rule) = &module.build {
                            ...
                            let status = if cfg!(windows) {
                                Command::new("cmd").args(["/C", &cmd_str]).current_dir(&system.base_dir).status()
                            } else {
                                Command::new("sh").arg("-c").arg(&cmd_str).current_dir(&system.base_dir).status()
                            };
        ...
        if meta.interpreter.is_none() && (meta.filename.ends_with(".c") || meta.filename.ends_with(".cpp")) {
            return format!("ERROR: Binary for '{}' was not compiled.", trigger);
        }
        ...
        if !file_path.exists() { return format!("ERROR: Not found: {:?}", file_path); }
